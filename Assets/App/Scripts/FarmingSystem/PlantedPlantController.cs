...
public class PlantedPlantController : MonoBehaviour
{
    private int _minutesNeed = 0;
    private int _currentStage = 0;
    private int _minutesPassed = 0;

    private float GrowthProgress => (float)_minutesPassed / _minutesNeed;
    private bool CanGrow => _land != null && _land.IsPlowed && _land.IsWatered && !_isAlive;
    ...
    {
        if (_plantData == null || _land == null) return;
    ...
    {
        foreach (var plantPrefab in _plantPrefabs)
        {
            Destroy(plantPrefab);
        }
        _plantPrefabs.Clear();
    ...
    {
        return _plantData != null && GrowthProgress >= 1 && !_isAlive;
    }
